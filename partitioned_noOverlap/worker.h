...
void redistribute_R(int rank)
{
	...
	for(int i = 0; i < worker_N - 1; ++i)
	{
		...
		MPI_Sendrecv(&R_send_amount,1,MPI_INT,commID,3,&R_recv_amount,1,MPI_INT,commID,3,MPI_COMM_WORLD,&status);

		PIX_NODE *ref_send_buffer = (PIX_NODE*)malloc(sizeof(PIX_NODE) * R_send_amount);
		PIX_NODE *ref_recv_buffer = (PIX_NODE*)malloc(sizeof(PIX_NODE) * R_recv_amount);
		...
		transfer += diffTime(start,end) * 0.001;
		...
		for(int kk = 0; kk < 2; ++kk)
		{
			...
			else if((kk == 0 && commID < rank) || (kk == 1 && commID > rank))
			{
				for(int j = 0; j < R_recv_ite; ++j)
				{
					...
					MPI_Recv(ref_recv_buffer + j * MPI_MESSLEN,cur_N,mpi_node,commID,3,MPI_COMM_WORLD,&status);
					//					printf("rank-%d recv from rank-%d R_chunk-%d\n",rank,commID,j);
				}
			}
		}
		gettimeofday(&end,NULL);
		printf("rank-%d communication ite-%d costs %.3f s\n",rank,i,diffTime(start,end) * 0.001);
		communication += diffTime(start,end) * 0.001;

		gettimeofday(&start,NULL);
		tbb::parallel_sort(ref_recv_buffer,ref_recv_buffer + R_recv_amount,cmp);
		gettimeofday(&end,NULL);
		printf("rank-%d sort reference costs %.3f s\n",rank,diffTime(start,end) * 0.001);

		sort += diffTime(start,end) * 0.001;

		gettimeofday(&start,NULL);
		if(R_recv_amount > sam_CM_N)
			singleCM(ref_recv_buffer,R_recv_amount,h_sam_node,sam_CM_N,rank);
		else
			singleCM(h_sam_node,sam_CM_N,ref_recv_buffer,R_recv_amount,rank);
		gettimeofday(&end,NULL);
		printf("rank-%d singleCM-%d costs %.3f s\n",rank,i,diffTime(start,end) * 0.001);
		sj += diffTime(start,end) * 0.001;

		free(ref_send_buffer);
		free(ref_recv_buffer);
	...
	gettimeofday(&start,NULL);
	tbb::parallel_sort(ref_buffer,ref_buffer + Rchunk_size[rank],cmp);
	gettimeofday(&end,NULL);
	printf("rank-%d sort reference costs %.3f s\n",rank,diffTime(start,end) * 0.001);
	sort += diffTime(start,end) * 0.001;

	gettimeofday(&start,NULL);
	if(Rchunk_size[rank] > sam_CM_N)
		singleCM(ref_buffer,Rchunk_size[rank],h_sam_node,sam_CM_N,rank);
	else
		singleCM(h_sam_node,sam_CM_N,ref_buffer,Rchunk_size[rank],rank);
	gettimeofday(&end,NULL);
	printf("rank-%d singleCM-%d costs %.3f s\n",rank,5,diffTime(start,end) * 0.001);
	sj += diffTime(start,end) * 0.001;

	printf("rank-%d communication %.3f s\n",rank,communication);
	printf("rank-%d transfer %.3f s\n",rank,transfer);
	printf("rank-%d sort %.3f s\n",rank,sort);
