...
void redistribute_R_bak(int rank)
{
	...
	for(int i = 0; i < worker_N - 1; ++i)
	{
		...
		gettimeofday(&end,NULL);
		waiting_cost += diffTime(start,end) * 0.001;
		...
		gettimeofday(&end,NULL);
		communication_cost += diffTime(start,end) * 0.001;
		printf("rank-%d communication cost single rond %.3f s\n",rank,diffTime(start,end) * 0.001);
		free(ref_buffer);
	}
	printf("rank-%d send_R communication cost %.3f s\n",rank,communication_cost);
	printf("rank-%d waiting cost %.3f s\n",rank,waiting_cost);

	transfer_ref(rank,rank,h_ref_node + ref_CM_N - Rchunk_size[rank],Rchunk_size[rank]);
	free(h_ref_ra);
	free(h_ref_dec);
	free(h_ref_range);
}

void redistribute_S_bak(int rank)
{
	MPI_Status status;
