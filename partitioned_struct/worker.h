...
void worker_requestSample(int rank)
{
	...
	//	MPI_Irecv(h_worker_sam + i * MPI_MESSLEN,len,mpi_node,MASTER_NODE,3,MPI_COMM_WORLD,&recv_request[request_cnt++]);
...
void worker_ownCM(int rank)
{
	...
	for(int i = 0; i < 100; ++i)
		printf("%d %.3lf %.3lf %d\n",i,h_worker_sam[i].ra,h_worker_sam[i].dec,h_worker_sam[i].pix);
	printf("-------------------- first 100 sam\n");
	for(int i = worker_sam_N - 100; i < worker_sam_N; ++i)
		printf("%d %.3lf %.3lf %d\n",i,h_worker_sam[i].ra,h_worker_sam[i].dec,h_worker_sam[i].pix);
	printf("-------------------- last 100 sam\n");
	for(int i = pix_chunk_startPos[0]; i < pix_chunk_startPos[0] + 100; ++i)
		printf("%d %.3lf %.3lf %d\n",i,h_ref_dup_node[i].ra,h_ref_dup_node[i].dec,h_ref_dup_node[i].pix);
	printf("-------------------- first 100 ref\n");
	for(int i = pix_chunk_startPos[0] + worker_ref_N - 100; i < pix_chunk_startPos[0] + worker_ref_N; ++i)
		printf("%d %.3lf %.3lf %d\n",i,h_ref_dup_node[i].ra,h_ref_dup_node[i].dec,h_ref_dup_node[i].pix);
	printf("-------------------- last 100 ref\n");
