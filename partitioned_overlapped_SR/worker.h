...
//multi-stream computeSI version

void computeSI_MS(double search_radius)
{
	int part_ref_N = ref_N / GPU_N;
#pragma omp parallel num_threads(GPU_N)
	{
		...
		checkCudaErrors(cudaMalloc((void**)&d_ref_ra[i],sizeof(double) * part_N));
		checkCudaErrors(cudaMalloc((void**)&d_ref_dec[i],sizeof(double) * part_N));
		checkCudaErrors(cudaMalloc((void**)&d_ref_range[i],sizeof(int) * part_N * MAX_RANGE_PAIR));
...
void count_sam(int rank)
{
	h_S_cnt = (int*)malloc(sizeof(int) * cntSize);
	memset(h_S_cnt,0,sizeof(int) * cntSize);

	for(int i = 0;i < sam_N; ++i)
		h_S_cnt[sam_node_buffer[i].pix]++;
}
...
void worker_gather(int rank)
{
	...
	if(rank == worker_root)
	{
		long long *cnt_sum = (long long*)malloc(sizeof(long long) * cntSize);
		int *Rcnt_buffer = (int*)malloc(sizeof(int) * cntSize);
		int *Scnt_buffer = (int*)malloc(sizeof(int) * cntSize);

#pragma omp parallel for
		for(int i = 0; i < cntSize; ++i)
			cnt_sum[i] = (long long)h_R_cnt[i] * h_S_cnt[i];

		printf("rank-%d  before gather\n",rank);
		for(int j = 1; j < numprocs; ++j)
		{
			MPI_Recv(Rcnt_buffer,cntSize,MPI_INT,j,3,MPI_COMM_WORLD,&status);
			MPI_Recv(Scnt_buffer,cntSize,MPI_INT,j,3,MPI_COMM_WORLD,&status);
			printf("rank-%d recv from rank-%d cntChunk\n",rank,j);
#pragma omp parallel for
			for(int i = 0; i < cntSize; ++i)
				cnt_sum[i] += (long long)Rcnt_buffer[i] * Scnt_buffer[i];
		}
...
//		printf("rank-%d  before gather\n",rank);
		...
		MPI_Send(h_S_cnt,cntSize,MPI_INT,worker_root,3,MPI_COMM_WORLD);
//		printf("rank-%d  after gather\n",rank);
		MPI_Bcast(chunk_start_pix,worker_N,MPI_INT,worker_root,MPI_COMM_WORLD);
		MPI_Bcast(chunk_end_pix,worker_N,MPI_INT,worker_root,MPI_COMM_WORLD);
	}
...
//			printf("rank-%d ref_CM_N %d sam_CM_N %d\n",rank,sam_CM_N,sam_CM_N);
	...
	gettimeofday(&end,NULL);
	printf("rank-%d communication cost %.3f s\n",rank,diffTime(start,end) * 0.001);
	memcpy(h_sam_node + sam_CM_N - Schunk_size[rank],sam_node_buffer + S_startPos[rank],sizeof(PIX_NODE) * Schunk_size[rank]);
...
#pragma omp atomic
			...
			int CM_ite = (cur_ref_N - 1) / part_ref_N + 1;
			for(int k = 0; k < CM_ite; ++k)
			{
				int ref_N = (k == (CM_ite - 1))?(cur_ref_N - part_ref_N * k):part_ref_N;
				checkCudaErrors(cudaMemcpy(d_ref_node[i],h_ref_node + start_ref_pos + k * part_ref_N,ref_N * sizeof(PIX_NODE), cudaMemcpyHostToDevice));
				kernel_singleCM<<<grid,block>>>(d_ref_node[i],ref_N,d_sam_node[i],cur_sam_N,d_sam_match[i],d_sam_matchedCnt[i],start_ref_pos,start_sam_pos);
			}
...
void exchange_and_crossmatch(int rank)
{
	struct timeval start,end;
	MPI_Status status;

	PIX_NODE *ref_recv_buffer[2];
	PIX_NODE *ref_send_buffer;

	ref_recv_buffer[0] = (PIX_NODE *)malloc(sizeof(PIX_NODE) * Rchunk_size[rank]);
	ref_recv_buffer[1] = (PIX_NODE *)malloc(sizeof(PIX_NODE) * Rchunk_size[rank]);
	ref_send_buffer = (PIX_NODE *)malloc(sizeof(PIX_NODE) * Rchunk_size[rank]);

	int R_send_amount;
	int R_recv_amount[2];

	R_recv_amount[0] = Rchunk_size[rank];
	gettimeofday(&start,NULL);
	transfer_ref(rank,rank,ref_recv_buffer[0],R_recv_amount[0]);
	gettimeofday(&end,NULL);
	printf("rank-%d transfer_ref costs %.3f s\n",rank,diffTime(start,end) * 0.001);
...
#pragma omp parallel num_threads(2)
		{
			int threadId = omp_get_thread_num() % 2;
			if(threadId == (i % 2))
			{
				struct timeval start,end;
				gettimeofday(&start,NULL);
				tbb::parallel_sort(ref_recv_buffer[threadId],ref_recv_buffer[threadId] + R_recv_amount[threadId],cmp);
				gettimeofday(&end,NULL);
				printf("rank-%d threadId-%d sort reference costs %.3f s\n",rank,threadId,diffTime(start,end) * 0.001);

				gettimeofday(&start,NULL);
				singleCM(ref_recv_buffer[threadId],R_recv_amount[threadId],h_sam_node,sam_CM_N,rank);
				gettimeofday(&end,NULL);
				printf("rank-%d thread-%d singleCM-%d costs %.3f s\n",rank,threadId,i,diffTime(start,end) * 0.001);
			...
			else
			{
				...
				R_send_amount = Rchunk_size[commID];

				MPI_Sendrecv(&R_send_amount,1,MPI_INT,commID,3,&R_recv_amount[threadId],1,MPI_INT,commID,3,MPI_COMM_WORLD,&status);

				ref_send_buffer = (PIX_NODE*)realloc(ref_send_buffer,sizeof(PIX_NODE) * R_send_amount);
				ref_recv_buffer[threadId] = (PIX_NODE*)realloc(ref_recv_buffer[threadId],sizeof(PIX_NODE) * R_recv_amount[threadId]);

				int R_send_ite = (R_send_amount - 1) / MPI_MESSLEN + 1;
				int R_recv_ite = (R_recv_amount[threadId] - 1) / MPI_MESSLEN + 1;

				gettimeofday(&start,NULL);
				transfer_ref(rank,commID,ref_send_buffer,Rchunk_size[commID]);
				gettimeofday(&end,NULL);
				printf("rank-%d transfer_ref costs %.3f \n",rank,diffTime(start,end) * 0.001);
				...
				for(int kk = 0; kk < 2; ++kk)
				{
					if((kk == 0 && commID > rank) || (kk == 1 && commID < rank))
					{
						for(int j = 0; j < R_send_ite; ++j)
						{
							...
							MPI_Send(ref_send_buffer + j * MPI_MESSLEN,cur_N,mpi_node,commID,3,MPI_COMM_WORLD);
					...
					else if((kk == 0 && commID < rank) || (kk == 1 && commID > rank))
					{
						for(int j = 0; j < R_recv_ite; ++j)
						{
							int cur_N;
							if(j == R_recv_ite - 1)
								cur_N = R_recv_amount[threadId] - j * MPI_MESSLEN;
							else
								cur_N = MPI_MESSLEN;

							MPI_Recv(ref_recv_buffer[threadId] + j * MPI_MESSLEN,cur_N,mpi_node,commID,3,MPI_COMM_WORLD,&status);
				...
				gettimeofday(&end,NULL);
				printf("rank-%d thread-%d communication costs %.3f \n",rank,threadId,diffTime(start,end) * 0.001);
			}
		}
	}

	gettimeofday(&end,NULL);
	printf("rank-%d 5 singleCM costs %.3f s\n",rank,diffTime(start,end) * 0.001);

	free(ref_recv_buffer[0]);
	free(ref_send_buffer);
	free(h_ref_ra);
	free(h_ref_dec);
	free(h_ref_range);
	...
	singleCM(ref_recv_buffer[threadId],R_recv_amount[threadId],h_sam_node,sam_CM_N,rank);
	gettimeofday(&end,NULL);
	printf("rank-%d thread-%d singleCM-5 costs %.3f s\n",rank,threadId,diffTime(start,end) * 0.001);
}
